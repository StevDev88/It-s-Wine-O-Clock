// Toastify the error and success message.
// Grab and edit wine based on user ID.
// Move to pop up instead of whole page.
// Display card of wine to be edited.
...
export async function getStaticPaths() {
	...
	let winesFromServer = await wines.json()
	return {
		paths: winesFromServer.map((wine) => {
			return {
				params: {
					id: wine._id,
				},
...
export default function EditWine({
	...
	const [wineId, setWineId] = useState(_id)
	...
	const handleSubmit = async (e) => {
		...
		if (wineName && wineDescription) {
			try {
				let response = await fetch(
					`http://localhost:3000/api/editWine?id=${wineId}`,
					{
						method: "PUT",
						body: JSON.stringify({
							_id: _id,
	...
	return (
		<>
			<form onSubmit={handleSubmit} className='form'>
				{error ? (
					<div className='alert-error'>{error.toString()}</div>
				) : null}
				{message ? (
					<div className='alert-message'>{message.toString()}</div>
				) : null}
